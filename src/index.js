...
function hasLevel2CraftingAccess(member, account) {
  return Boolean(member?.roles?.cache?.has(CRAFTING_LEVEL2_ACCESS_ROLE_ID));
}
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
      ...
      if (interaction.commandName === "استخراج-طاولة-تصنيع") {
        ...
        if (levelKey === CRAFTING_TABLE_LEVELS.level2.key) {
          if (!hasLevel2CraftingAccess(interaction.member, account)) {
            await interaction.reply({
              content: `لا يمكنك استخراج أو استخدام طاولة المستوى الثاني إلا إذا كنت تملك الرتبة <@&${CRAFTING_LEVEL2_ACCESS_ROLE_ID}>.`,
              ephemeral: true
            });
            return;
          }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === "resources_main_menu") {
      ...
      if (action === "transfer") {
        await interaction.showModal(createResourceTransferModal());
        return;
      }
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith("activation_open:")) {
        await interaction.showModal(createActivationModal(createActivationRequestId()));
        return;
      }
      ...
      if (interaction.customId.startsWith("crafting_begin_purchase:")) {
        ...
        if (!level?.purchasable) {
          await interaction.reply({ content: "هذا المستوى غير متاح للبيع حاليًا.", ephemeral: true });
          return;
        }

        if (hasCraftingTable(account, 1) && levelKey === CRAFTING_TABLE_LEVELS.level1.key) {
          await interaction.reply({ content: "أنت تملك طاولة تصنيع مستوى أول بالفعل.", ephemeral: true });
          return;
        }

        const cooldownText = getCraftingCooldownText(account);
        if (cooldownText) {
          await interaction.reply({ content: `لا يمكنك إعادة المحاولة الآن. الموعد القادم ${cooldownText}.`, ephemeral: true });
          return;
        }
    ...
    if (interaction.type === InteractionType.ModalSubmit) {
      ...
      if (interaction.customId.startsWith("modal_activation:")) {
        if (interaction.member?.roles?.cache?.has(ACTIVATION_ROLE_ID)) {
          await interaction.reply({ content: "أنت مفعّل بالفعل داخل عرب وورلد.", ephemeral: true });
          return;
        }
        ...
        if (!fullName || !age || !robloxUsername || !pledge) {
          await interaction.reply({ content: "يرجى تعبئة جميع الحقول المطلوبة.", ephemeral: true });
          return;
        }
...
app.post("/web/showroom-request-code", async (req, res) => {
  try {
    ...
    await client.users.fetch(account.discordUserId).then((user) => user.send({
      embeds: [
        new EmbedBuilder()
          .setColor(0x0b1f3a)
          .setTitle("🔐 رمز تحقق دخول المعرض")
          .setDescription("**تم طلب تسجيل دخول جديد إلى موقع معرض السيارات. استخدم رمز التحقق التالي داخل الموقع لإكمال الدخول.**")
          .addFields(
          ...
          .setFooter({ text: "Arab World Showroom • Verification" })
          .setTimestamp()
      ]
    }).catch(() => null)).catch(() => null);
...
app.post("/web/showroom-verify-code", async (req, res) => {
  try {
    if (!isAuthorizedInternalRequest(req)) {
      return res.status(401).json({ ok: false, error: "unauthorized" });
    }
...
app.post("/web/account-login", async (req, res) => {
  try {
    ...
    if (!robloxUsername || !bankAccountId) {
    ...
    if (String(account.robloxUsername || "").trim().toLowerCase() !== robloxUsername.toLowerCase()) {
      return res.status(403).json({ ok: false, error: "username_mismatch" });
    }

    return res.status(200).json({
      ok: true,
      ...buildWebsiteAccountSnapshot(account)
    });
  } catch (error) {
...
app.post("/web/account-balance", async (req, res) => {
  try {
    ...
    const bankAccountId = String(req.body?.bankAccountId || req.body?.accountNumber || "").trim();
    if (!bankAccountId) {
      return res.status(400).json({ ok: false, error: "missing_account_number" });
    }
...
app.post("/web/buy-car", async (req, res) => {
  try {
    ...
    if (!bankAccountId || !robloxUsername || !vehicleName) {
...
app.post("/web/account-transfer", async (req, res) => {
  try {
    ...
    if (!bankAccountId || !robloxUsername || !Number.isFinite(amount)) {
      return res.status(400).json({ ok: false, error: "missing_transfer_fields" });
    }

    const senderAccount = findAccountByAccountNumber(bankAccountId);
    if (!senderAccount) {
      return res.status(404).json({ ok: false, error: "account_not_found" });
    }

    if (String(senderAccount.robloxUsername || "").trim().toLowerCase() !== robloxUsername.toLowerCase()) {
      return res.status(403).json({ ok: false, error: "username_mismatch" });
    }
    ...
    if (!result.ok) {
      ...
      return res.status(statusMap[result.error] || 400).json({
        ...
        account: buildWebsiteAccountSnapshot(getAccount(senderAccount.discordUserId))
      });
    }
...
app.post("/web/name-change-request", async (req, res) => {
  try {
    ...
    if (!bankAccountId || !oldName || !newName || !robloxUsername) {
