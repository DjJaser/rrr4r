...
export function registerWebsiteRoutes(app, deps) {
  ...
  async function sendWebsiteVerificationDm(discordUserId, payload) {
    const linkedMember = await findGuildMemberForWebsiteAccess(discordUserId).catch(() => null);
    if (linkedMember?.user) {
      return withTimeout(
        () => linkedMember.user.send(payload)
          .then(() => ({ ok: true, delivery: "guild_member" }))
