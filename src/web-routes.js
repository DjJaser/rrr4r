export function registerWebsiteRoutes(app, deps) {
  ...
  app.post(["/web/showroom-request-code", "/web/mobile-request-code"], async (req, res, next) => {
    try {
      ...
      ).then((member) => ({ ok: true, member })).catch((error) => ({ ok: false, error: error?.message || "guild_member_lookup_failed" }));

      if (!memberResult.ok && memberResult.error === "guild_member_lookup_timeout") {
      ...
      ).then((user) => ({ ok: true, user })).catch((error) => ({ ok: false, error: error?.message || "discord_user_fetch_failed" }));
