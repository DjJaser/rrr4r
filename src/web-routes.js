export function registerWebsiteRoutes(app, deps) {
  ...
  async function resolveDiscordUserForWebsiteAccess(discordUserId) {
    ...
    const memberUserResult = await withTimeout(
      async () => {
        const member = await findGuildMemberForWebsiteAccess(discordUserId);
        return member?.user || null;
      },
      5000,
      "guild_member_user_lookup_timeout"
    ).then((user) => ({ ok: true, user }))
      .catch((error) => ({ ok: false, error: error?.message || "guild_member_user_lookup_failed" }));

    if (memberUserResult.ok && memberUserResult.user) {
      return memberUserResult;
    }

    const fetchedUserResult = await withTimeout(
      () => client.users.fetch(discordUserId, { force: false }).catch(() => null),
      8000,
      "discord_user_fetch_timeout"
    ).then((user) => ({ ok: true, user }))
      .catch((error) => ({ ok: false, error: error?.message || "discord_user_fetch_failed" }));

    if (fetchedUserResult.ok && fetchedUserResult.user) {
      return fetchedUserResult;
    }

    if (!memberUserResult.ok && memberUserResult.error === "guild_member_user_lookup_timeout") {
      return { ok: false, error: "guild_member_user_lookup_timeout" };
    }

    if (!fetchedUserResult.ok && fetchedUserResult.error === "discord_user_fetch_timeout") {
  ...
  app.post(["/web/showroom-request-code", "/web/mobile-request-code"], async (req, res, next) => {
    try {
      ...
      const userResult = await resolveDiscordUserForWebsiteAccess(account.discordUserId);

      if (!userResult.ok && userResult.error === "guild_member_user_lookup_timeout") {
        pendingWebsiteLoginVerifications.delete(verificationId);
        return res.status(504).json({ ok: false, error: "guild_member_user_lookup_timeout" });
      }

      if (!userResult.ok && userResult.error === "discord_user_fetch_timeout") {
        pendingWebsiteLoginVerifications.delete(verificationId);
  ...
  app.post(["/web/showroom-request-code", "/web/mobile-request-code"], async (req, res) => {
    try {
      if (!isAuthorizedInternalRequest(req)) {
        return res.status(401).json({ ok: false, error: "unauthorized" });
      }
  ...
  app.post(["/web/showroom-verify-code", "/web/mobile-verify-code"], async (req, res) => {
    try {
      if (!isAuthorizedInternalRequest(req)) {
        return res.status(401).json({ ok: false, error: "unauthorized" });
      }
